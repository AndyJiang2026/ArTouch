...
function handlePreview(row) {
  if (row.file_path) {
    // Convert local server path to accessible URL
    // e.g. /root/ArTouch/backend/videos/002_d5aace2f.mp4 → /videos/002_d5aace2f.mp4
    const filename = row.file_path.split('/').pop()
    previewUrl.value = `/videos/${filename}`
  } else {
    previewUrl.value = ''
  }
